...
app = FastAPI(
    title="Queryable — Text-to-SQL API",
...
def require_db_pool():
    ...
    if not pool:
        raise HTTPException(
            ...
            detail="Database connection is not available. Please verify DATABASE_URL in backend/.env"
        )
    return pool
...
async def health_check():
    ...
    return {
        ...
        "supported_formats": [".csv", ".tsv", ".parquet", ".sql", ".json", ".jsonl", ".xlsx", ".sqlite", ".db"]
    }
...
async def upload_dataset(
    ...
    """
    Streams and ingests database files (CSV, Parquet, SQL, JSON, Excel, SQLite).
    Supports large files up to 50 GB with disk streaming.
    """
    pool = require_db_pool()
    ...
    dest_filename = f"{int(time.time())}_{sanitize_identifier(file.filename)}{Path(file.filename).suffix}"
    dest_path = TEMP_UPLOAD_DIR / dest_filename

    try:
        # 1. Stream file directly to disk
        bytes_written = await save_upload_to_disk(file, dest_path)
        logger.info("Saved %d bytes to %s", bytes_written, dest_path)

        # 2. Process and ingest into PostgreSQL
        result = await process_file_ingestion(
            file_path=dest_path,
        ...
        # 3. Synchronize new table to DuckDB engine
        try:
        ...
        raise HTTPException(status_code=400, detail=f"Data ingestion failed: {exc}")
    finally:
